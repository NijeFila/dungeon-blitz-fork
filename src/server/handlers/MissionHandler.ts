...
export class MissionHandler {
    ...
    private static readonly DUNGEON_COMPLETION_CLOSE_REORDER_TOLERANCE_MS = 15000;
    ...
    static async handleForcedDungeonBossCompletion(client: Client, destroyedEntity: any): Promise<void> {
        ...
        const lastCutsceneEndAt = Math.max(0, Number(client.lastDungeonCutsceneEndAt ?? 0));
        const closeObservedForScope =
            String(client.lastDungeonCutsceneEndScope ?? '').trim() === levelScope &&
            lastCutsceneEndAt > 0 &&
            Date.now() - lastCutsceneEndAt <= MissionHandler.DUNGEON_COMPLETION_CLOSE_REORDER_TOLERANCE_MS;
