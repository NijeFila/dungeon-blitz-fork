...
function createCapstoneLargeEye(): any {
    return {
        id: 6975772,
        name: 'NephitLargeEye',
        characterName: ',NephitLargeEye',
        character_name: ',NephitLargeEye',
        isPlayer: false,
        roomId: 2100375684,
        team: EntityTeam.ENEMY,
        entState: EntityState.ACTIVE,
...
/**
 * Replays the signals from the reported Capstone run. Stage-one and final-room
 * cinematics are not completion signals. The authored NephitLargeEye death is
 * the objective, and its following defeat cinematic is the completion gate.
 */
async function testCapstoneCompletesFromActualLargeEyeDefeat(): Promise<void> {
    const client = createBossDungeonClient('CapstoneRunner', 83008, 'AC_Mission6');
    const missionId = Number(MissionLoader.findPrimaryMissionByDungeon('AC_Mission6')?.MissionID ?? 0);

    MissionHandler.noteDungeonCutsceneStart(client as never, 1867066964);
    MissionHandler.noteDungeonCutsceneEnd(client as never, 1867066964);
    MissionHandler.noteDungeonCutsceneStart(client as never, 1867066964);
    MissionHandler.noteDungeonCutsceneEnd(client as never, 1867066964);
    assert.equal(rankPacketCount(client), 0, 'stage-one cinematics completed the Capstone');

    const boss = createCapstoneLargeEye();
    seedSingleBossRun(client, boss);
    MissionHandler.noteDungeonCutsceneStart(client as never, boss.roomId);
    MissionHandler.noteDungeonCutsceneEnd(client as never, boss.roomId);
    assert.equal(rankPacketCount(client), 0, 'the final-room intro completed the Capstone');
    const scope = getClientLevelScope(client as never);
    const introState = GlobalState.dungeonCompletions.get(scope)?.cutscenesByRoom.get(boss.roomId);
    assert.ok(introState, 'the final-room intro was not recorded');
    // The live fight lasts well beyond the packet-reordering tolerance used for
    // a death racing its own cinematic close.
    introState.endedAt = Date.now() - 60_000;
    client.lastDungeonCutsceneEndAt = introState.endedAt;
    ...
    await MissionHandler.handleForcedDungeonBossCompletion(client as never, boss);
    assert.equal(rankPacketCount(client), 0, 'the plate appeared before the large-eye defeat cinematic');

    MissionHandler.noteDungeonCutsceneStart(client as never, boss.roomId);
    MissionHandler.noteDungeonCutsceneEnd(client as never, boss.roomId);

    assert.equal(rankPacketCount(client), 1, 'the large-eye defeat did not open the Capstone rank plate');
    assert.ok(
        missionId > 0 && Number(client.character.missions[String(missionId)]?.state ?? 0) >= 2,
        'the large-eye defeat did not credit the Capstone mission'
    );
}
...
async function main(): Promise<void> {
    ...
    try {
        ...
        GlobalState.sessionsByToken.clear();
        GlobalState.dungeonCompletions.clear();
        await testCapstoneCompletesFromActualLargeEyeDefeat();
